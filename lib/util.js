 ...
 * @param {{BadestelleLink: string; ProfilLink: string; Farbe: string; Wasserqualitaet:string ; Algen: string, Farb_ID: number}[]} data
 */
const merge = (data, predictions) => {
  data.forEach((d) => {
    ...
    if (id in predictions) {
      ...
      if (result !== quality) {
        d.Wasserqualitaet = String(result);
        d.Farb_ID = result;
 ...
 * @returns number
 */
const showdown = (lageso, _algae, prediction) => {
  let newState = lageso;
  if (
    prediction === "mangelhaft" &&
    ![3, 4, 5, 6, 13, 14, 15, 16].includes(lageso)
  ) {
...
/**
 * @param {number} stateStr
 */
const colorFromState = (stateStr) => {
  const state = parseInt(stateStr.toString());
  if ([1, 11, 2, 12].includes(state)) {
    return "green";
  }
  if ([3, 13, 4, 14, 10].includes(state)) {
    return "orange";
  }
  if ([5, 15, 6, 16].includes(state)) {
    return "red";
  }
  return "grey";
};
...
const imageFromState = (state) => {
  switch (state) {
    case 1:
      ...
      break;
    case 11:
      return "gruen_prog.jpg";
      break;
    case 2:
      return "gruen_a.jpg";
      break;
    case 12:
      return "gruen_a_prog.jpg";
      break;
    case 3:
      return "gelb.jpg";
      break;
    case 13:
      return "gelb_prog.jpg";
      break;
    case 4:
      return "gelb_a.jpg";
      break;
    case 14:
      return "gelb_a_prog.jpg";
    case 5:
      return "rot.jpg";
      break;
    case 15:
